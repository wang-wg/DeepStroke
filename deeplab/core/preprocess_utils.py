...
def randomly_scale_image_and_label(image, skel, skel_35, label=None, scale=1.0, label_order=None):
  ...
  if scale == 1.0:
    return image, skel,skel_35, label, label_order
  image_shape = tf.shape(image)
  ...
  skel = tf.squeeze(tf.image.resize_bilinear(
      tf.expand_dims(skel, 0),
      new_dim,
      align_corners=True), [0])
  skel_35 = tf.squeeze(tf.image.resize_bilinear(
      tf.expand_dims(skel_35, 0),
      new_dim,
      align_corners=True), [0])
  ...
  if label_order is not None:
    label_order = tf.squeeze(tf.image.resize_nearest_neighbor(
        tf.expand_dims(label_order, 0),
        new_dim,
        align_corners=True), [0])
