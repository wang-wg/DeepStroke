...
def _get_data(data_provider, dataset_split):
  ...
  if common.LABELS_CLASS_ORDER not in data_provider.list_items():
    raise ValueError('Failed to find labels_order.')

  image, skel, skel_35, height, width ,stroke, stroke_order = data_provider.get(
      [common.IMAGE, common.SKEL,common.SKEL_35, common.HEIGHT, common.WIDTH, common.STROKE, common.STROKE_ORDER])
  ...
  if dataset_split != common.TEST_SET:
    ...
    label_order, = data_provider.get([common.LABELS_CLASS_ORDER])

  return image, label, skel, skel_35, image_name, height, width ,stroke, stroke_order, label_order
...
def get(dataset,
  ...
  data_provider = dataset_data_provider.DatasetDataProvider(
      ...
      shuffle=is_training)
  image, label, skel, skel_35, image_name, height, width ,stroke, stroke_order, label_order = _get_data(data_provider,
                                                      dataset_split)
  ...
  if label_order is not None:
    if label_order.shape.ndims == 2:
      label_order = tf.expand_dims(label_order, 2)
    elif label_order.shape.ndims == 3 and label_order.shape.dims[2] == 1:
      pass
    else:
      raise ValueError('Input label shape must be [height, width], or '
                       '[height, width, 1].')

    label_order.set_shape([None, None, 1])

  original_image, image, original_skel, skel,original_skel_35,skel_35,label, label_order = input_preprocess.preprocess_image_and_label(
      image,
      skel,
      skel_35,
      label,
  ...
  sample = {
      ...
      common.STROKE_ORDER: stroke_order,
      common.SKEL:skel,
      common.SKEL_35:skel_35
  }
  if label is not None:
    sample[common.LABEL] = label

  if label is not None:
    sample[common.LABEL_ORDER] = label_order

  if not is_training:
    # Original image is only used during visualization.
    sample[common.ORIGINAL_IMAGE] = original_image,
    num_threads = 1
    sample[common.ORIGINAL_SKEL] = original_skel,
    num_threads = 1
