...
def predict_labels(model_options, images, skels_35=None, skels=None, stroke=None, stroke_order=None, image_pyramid=None):
  ...
  for output in sorted(outputs_to_scales_to_logits):
    ...
    logits = tf.image.resize_bilinear(
        ...
        align_corners=True)
    ## Inference module 
    image_feature=(2.0 / 255.0) * tf.to_float(images)-1
    logits_fix=tf.concat([tf.expand_dims(1000000*image_feature[:,:,:,0],3),logits[:,:,:,1:]],3)
#    logits_tmp=logits[:,:,:,0]+tf.expand_dims(image_feature[:,:,:,0],3)
#    logits_fix=tf.concat([logits_tmp,logits_tmp],3)
    predictions[output] = tf.argmax(logits_fix, 3)
...
def multi_scale_logits(model_options,
                       image_pyramid,
                       images,
                       skels_35 = None,
                       skels = None,
  ...
  # add skel information
#  images=tf.concat([images,skels],3)
  ...
  for count, image_scale in enumerate(image_pyramid):
    if image_scale != 1.0:
      ...
      scaled_skels=tf.image.resize_bilinear(skels,scaled_crop_size,align_corners=True)
      scaled_skels_35=tf.image.resize_bilinear(skels_35,scaled_crop_size,align_corners=True)
      if model_options.crop_size:
        scaled_images.set_shape([None, scaled_height, scaled_width, 3])
        scaled_skels.set_shape([None, scaled_height, scaled_width, 1])
        scaled_skels_35.set_shape([None, scaled_height, scaled_width, 1])
    else:
      scaled_crop_size = model_options.crop_size
      scaled_images = images
      scaled_skels=skels
      scaled_skels_35 = skels_35

    updated_options = model_options._replace(crop_size=scaled_crop_size)
    outputs_to_logits = _get_logits(
        updated_options,
        images=scaled_images,
        skels_35=scaled_skels_35,
        skels=scaled_skels,
        stroke=stroke,
        stroke_order=stroke_order,
...
def _extract_features(model_options,
                      images,
                      skels_35,
                      skels,
                      stroke,
...
#       # add feature to stroke segmentation branch
        stroke_feature_1=tf.image.resize_bilinear(stroke,[pool_height,pool_width],align_corners=True)
        stroke_feature_1.set_shape([None, pool_height, pool_width, 35])

        stroke_feature=tf.image.resize_nearest_neighbor(skels_35,[pool_height,pool_width],align_corners=True)
        stroke_feature.set_shape([None, pool_height, pool_width, 1])
        stroke_feature=tf.squeeze(tf.one_hot(tf.cast(stroke_feature,tf.int32),35),[3])

        # add feature to stroke order labeling branch 
        skels_feature_1=tf.image.resize_bilinear(stroke_order,[pool_height,pool_width],align_corners=True)
        skels_feature_1.set_shape([None, pool_height, pool_width, 32])

        skels_feature=tf.image.resize_nearest_neighbor(skels,[pool_height,pool_width],align_corners=True)
        skels_feature.set_shape([None, pool_height, pool_width, 1])
        skels_feature=tf.squeeze(tf.one_hot(tf.cast(skels_feature,tf.int32),32),[3])

        # match and fusion module apply in stroke segmentation branch 
        concat_logits = tf.concat(branch_logits+[stroke_feature,stroke_feature_1], 3)
#        concat_logits = slim.conv2d(
#            concat_logits, depth, 1, scope=_CONCAT_PROJECTION_SCOPE)
        concat_logits = slim.conv2d(
            concat_logits, depth, 3, scope=_CONCAT_PROJECTION_SCOPE)
        concat_logits = slim.conv2d(
            concat_logits, depth, 3, scope=_CONCAT_PROJECTION_SCOPE+'_1')
        ...
        # stroke order branch
        # match and fusion module apply in stroke order labeling branch 
        concat_logits_strokeOrder = tf.concat(branch_logits+[skels_feature,skels_feature_1], 3)
        concat_logits_strokeOrder = slim.conv2d(
            concat_logits_strokeOrder, depth, 3, scope=_CONCAT_PROJECTION_SCOPE+ '_strokeOrder')
        concat_logits_strokeOrder = slim.conv2d(
            concat_logits_strokeOrder, depth, 3, scope=_CONCAT_PROJECTION_SCOPE+ '_strokeOrder1')

        concat_logits_strokeOrder = slim.dropout(
            concat_logits_strokeOrder,
            keep_prob=0.9,
            is_training=is_training,
            scope=_CONCAT_PROJECTION_SCOPE + '_strokeOrder_dropout')

        return concat_logits,concat_logits_strokeOrder, end_points


def _get_logits(model_options,
                images,
                skels_35=None,
                skels=None,
                stroke=None,
  ...
  """
  features, features_stroke_order, end_points = _extract_features(
      model_options,
      images=images,
      skels_35=skels_35,
      skels=skels,
  ...
  if model_options.decoder_output_stride is not None:
    ...
    decoder_width = scale_dimension(model_options.crop_size[1],
                                    1.0 / model_options.decoder_output_stride)
    # stroke order branch add stroke imformation
    #features_stroke_order = tf.concat([features_stroke_order,features], 3)
    ...
    features_stroke_order = refine_by_decoder_order(
        features_stroke_order,
        end_points,
        decoder_height=decoder_height,
        decoder_width=decoder_width,
        skels_35=skels_35,
    ...
    #features_stroke_order = tf.concat([features_stroke_order,features], 3)
    # add skel information
#    skels_feature=tf.image.resize_bilinear(skels,[decoder_height,decoder_width],align_corners=True)
#    skels_feature=tf.squeeze(tf.one_hot(tf.cast(skels_feature,tf.int32),35),[3])
#    features=tf.concat([features,skels_feature],3)
  featrue_all = {}
  featrue_all['semantic'] = features
  featrue_all['stroke_order'] = features_stroke_order
  ...
  for output in sorted(model_options.outputs_to_num_classes):
    ...
    output_logits_shape_tmp=outputs_to_logits[output].get_shape()
    image_resize = tf.image.resize_bilinear(
                  images, [output_logits_shape_tmp[1], output_logits_shape_tmp[2]], align_corners=True)
    image_feature=(2.0 / 255.0) * tf.to_float(image_resize)-1
    outputs_to_logits[output]=tf.concat([tf.expand_dims(10*image_feature[:,:,:,0],3),outputs_to_logits[output][:,:,:,1:]],3)
...
def refine_by_decoder_order(features,
                            end_points,
                            decoder_height,
                            decoder_width,
                            skels_35=None,
          ...
          for i, name in enumerate(feature_list):
            ...
            decoder_features_list.append(
                slim.conv2d(
                    ...
                    48,
                    1,
                    scope='strokeorder_feature_projection' + str(i)))
            ...
            if decoder_use_separable_conv:
              decoder_features = _split_separable_conv2d(
                  ...
                  rate=1,
                  weight_decay=weight_decay,
                  scope='strokeorder_decoder_conv0')
              decoder_features = _split_separable_conv2d(
                  decoder_features,
                  filters=decoder_depth,
                  rate=1,
                  weight_decay=weight_decay,
                  scope='strokeorder_decoder_conv1')
            else:
              num_convs = 2
              decoder_features = slim.repeat(
                  tf.concat(decoder_features_list, 3),
                  ...
                  decoder_depth,
                  3,
                  scope='strokeorder_decoder_conv' + str(i))
          return decoder_features
