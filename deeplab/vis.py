...
from deeplab import model_DeepStroke
...
def _process_batch(sess, original_images,GT_images,GT_images_order,skels_images, semantic_predictions, semantic_predictions_order, image_names,
  ...
  (original_images,
   ...
   semantic_predictions_order,
   image_names,
   image_heights,
   image_widths) = sess.run([original_images,GT_images,GT_images_order,skels_images, semantic_predictions,semantic_predictions_order,
  ...
  for i in range(num_image):
    ...
    GT_image=np.squeeze(GT_images[i])
    GT_image_a=GT_image[:image_height, :image_width]
    #order
    GT_image_order=np.squeeze(GT_images_order[i])
    GT_image_order_a=GT_image_order[:image_height, :image_width]
    skels_images=np.squeeze(skels_images[i])
    skels_images=skels_images.astype(np.int32)
    #skels_images_a=skels_images[:image_height, :image_width]
    #skels_images_a=skels_images.astype(np.int32)

    semantic_prediction = np.squeeze(semantic_predictions[i])
    crop_semantic_prediction = semantic_prediction[:image_height, :image_width]

    semantic_prediction_order = np.squeeze(semantic_predictions_order[i])
    crop_semantic_prediction_order = semantic_prediction_order[:image_height, :image_width]

    image_filename = image_names[i]
    # Save GT.
#    save_annotation.save_annotation(
#        GT_image_a, save_dir, _IMAGE_FORMAT % image_filename,
#        add_colormap=True)
#    #order
#    save_annotation.save_annotation(
#        GT_image_order_a, save_dir+'_order', _IMAGE_FORMAT % image_filename,
#        add_colormap=True)#

#    # save skels_images
#    save_annotation.save_annotation(
#        skels_images, save_dir,
#        _SKEL_FORMAT % image_filename, add_colormap=True)
#    # Save prediction.
    save_annotation.save_annotation(
    ...
    save_annotation.save_annotation(
        crop_semantic_prediction_order, save_dir+'_order',
        _PREDICTION_FORMAT % image_filename, add_colormap=True,
        colormap_type=FLAGS.colormap_type)
    ...
    if FLAGS.also_save_raw_predictions:
      if train_id_to_eval_id is not None:
        crop_semantic_prediction = _convert_train_id_to_eval_id(
            ...
            train_id_to_eval_id)
        #order
        crop_semantic_prediction_order = _convert_train_id_to_eval_id(
            crop_semantic_prediction_order,
            train_id_to_eval_id)
      save_annotation.save_annotation(
      ...
      #order
      save_annotation.save_annotation(
          crop_semantic_prediction_order, raw_save_dir+'_order', image_filename,
          add_colormap=False)
...
def main(unused_argv):
  ...
  tf.gfile.MakeDirs(save_dir+'_order')
  raw_save_dir = os.path.join(
      FLAGS.vis_logdir, _RAW_SEMANTIC_PREDICTION_SAVE_FOLDER)
  tf.gfile.MakeDirs(raw_save_dir)
  tf.gfile.MakeDirs(raw_save_dir+'_order')
  ...
  with g.as_default():
    ...
    #print(samples)
    if FLAGS.model_name=='DualStrokeNet':
      outputs_to_num_classes={common.OUTPUT_TYPE: dataset.num_classes, 'stroke_order': 32}
    else:
    ...
    if tuple(FLAGS.eval_scales) == (1.0,):
      ...
      if FLAGS.model_name=='DeepStroke':
        predictions = model_DeepStroke.predict_labels(
          images = samples[common.IMAGE],
          skels_35 = samples[common.SKEL_35],
          skels = samples[common.SKEL],
          stroke = samples[common.STROKE],
          stroke_order = samples[common.STROKE_ORDER],
        ...
        predictions = model.predict_labels(
          ...
          model_options=model_options,
          image_pyramid=FLAGS.image_pyramid)    
    prediction = predictions[common.OUTPUT_TYPE]
    if FLAGS.model_name=='DeepStroke':
      predictions_strokeOrder = predictions[common.OUTPUT_TYPE]
    else:
      predictions_strokeOrder = predictions['stroke_order']
    ...
    if FLAGS.min_resize_value and FLAGS.max_resize_value:
      ...
      prediction = tf.squeeze(
          tf.image.resize_images(tf.expand_dims(prediction, 3),
                                 resized_shape,
                                 method=tf.image.ResizeMethod.NEAREST_NEIGHBOR,
                                 align_corners=True), 3)

      #predictions_strokeOrder = tf.slice(
#          predictions_strokeOrder,
#          [0, 0, 0],
#          [1, original_image_shape[0], original_image_shape[1]])
#      resized_shape = tf.to_int32([tf.squeeze(samples[common.HEIGHT]),
#                                   tf.squeeze(samples[common.WIDTH])])
#      predictions_strokeOrder = tf.squeeze(
#          tf.image.resize_images(tf.expand_dims(predictions_strokeOrder, 3),
#                                 resized_shape,
#                                 method=tf.image.ResizeMethod.NEAREST_NEIGHBOR,
#                                 align_corners=True), 3)
    ...
    while (FLAGS.max_number_of_iterations <= 0 or
      ...
      with sv.managed_session(FLAGS.master,
        ...
        for batch in range(num_batches):
          ...
          _process_batch(sess=sess,
                         ...
                         GT_images=samples[common.LABEL],
                         GT_images_order=samples[common.LABEL_ORDER],
                         skels_images=samples[common.SKEL_35],
                         semantic_predictions=prediction,
                         semantic_predictions_order=predictions_strokeOrder,
      ...
      time_end=time.time()
      print('totally cost',time_end-start)
      tf.logging.info(
